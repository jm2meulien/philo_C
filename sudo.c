...
void retourMatPrec(void)
{
   /* on supprime le dernier elt du chainage */
   if (chaine_suppr((void*)AdM) == 0) finerr("Matrice absente !?");
   free((void*)AdM);  /* on supprime physiquement la matrice */
   AdM = (char*)chaine_adresseDernier();
   return;
}
...
int i, j;
  ...
  chaine_ajout((void*)MAT);  /* ajout dans le chainage de la mat. de depart */
  affiche_mat();
#ifdef TRACE
